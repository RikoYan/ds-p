...
with col1:
    ...
    sns.barplot(
        y="cnt", 
        x="nameseason",
        data=season_11_sum_df.sort_values(by="season", ascending=True),
        palette=colors,
        ax=ax
    )
    ax.set_title("tahun 2011", loc="center", fontsize=50)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
...
with col2:
    ...
    sns.barplot(
        y="cnt", 
        x="nameseason",
        data=season_12_sum_df.sort_values(by="season", ascending=True),
        palette=colors,
        ax=ax
    )
    ax.set_title("tahun 2012", loc="center", fontsize=50)
    ax.set_ylabel(None)
    ax.set_xlabel(None)
    ax.tick_params(axis='x', labelsize=35)
    ax.tick_params(axis='y', labelsize=30)
